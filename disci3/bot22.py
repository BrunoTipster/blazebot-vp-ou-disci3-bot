...
class BlazeBot:
    ...
    async def _match_pattern(self, pattern: list, prediction: str) -> bool:
        ...
        if not res or "items" not in res:
            return False
        # Para padrões de branco, usa histórico completo (V+P+B)
        if prediction == "B":
            items_hist = [i for i in res["items"] if i["color"] in ("V", "P", "B")]
        else:
            items_hist = [i for i in res["items"] if i["color"] in ("V", "P")]
        items_reversed = items_hist[::-1]
        if len(pattern) > len(items_reversed):
            return False
        segment = items_reversed[-len(pattern):]
    ...
    def _evaluate_result(self, color_result: str) -> str:
        if color_result == self.bet.color:
            return "WIN"
        # Se apostamos em branco e não saiu branco -> LOSS direto (sem gale)
        if self.bet.color == "B":
            return "LOSS"
        if color_result == "B":
    ...
    async def _on_rolling(self, roll: int, color: str) -> None:
        ...
        if result == "WIN":
            ...
            payout          = COLOR_PAYOUT.get(prediction_used, 2.0)
            ...
            _lucro_win  = round(_val_apost * (COLOR_PAYOUT.get(prediction_used, 2.0) - 1), 2)
            ...
            log.info(f"WIN VP | cor={color} | gale={gale_at} | fonte={source}")
    ...
    async def _verificar_e_enviar_sinal(self) -> None:
        ...
        if match:
            ...
            if probs_mk and prediction != "B":  # Markov não cancela sinais de branco
